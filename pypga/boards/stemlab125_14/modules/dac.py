...
def Dac(bits=14):
    class _Dac(Module):
        out1: FixedPointRegister(
            width=bits, signed=True, decimals=bits - 1, readonly=True, default=0
        )
        out2: FixedPointRegister(
            width=bits, signed=True, decimals=bits - 1, readonly=True, default=0
        )
...
def ExampleDac(bits=14):
    class _ExampleDac(Dac(bits=bits)):
        out1_setting: FixedPointRegister(
            width=bits, signed=True, decimals=bits - 1, default=0
        )
        out2_setting: FixedPointRegister(
            width=bits, signed=True, decimals=bits - 1, default=0
        )

class Server:
    def __init__(self, host, delay=0.05):
        self._delay = delay
        self.shell = SshShell(
            hostname=host, sshport=22, user="root", password="root", scp=True
        )
        self.update_fpga()
        self.installserver()
    ...
    def update_fpga(self, filename):
        ...
        if source is None or not os.path.isfile(source):
            if source is not None:
                self.logger.warning(
                    'Desired bitfile "%s" does not exist. Using default file.', source
                )
            source = os.path.join(
                os.path.abspath(os.path.dirname(__file__)), "fpga", "red_pitaya.bin"
            )
        ...
        )  # for 0.94 and higher #needs test
        sleep(self.parameters["delay"])
        self.ssh.ask("ro")
    ...
    def startserver(self):
        ...
        if not "sh" in result:  # sh in result means we tried the wrong binary version
            self.logger.debug(
                "Server application started on port %d", self.parameters["port"]
            )

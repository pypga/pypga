...
class MigenPid(MigenModule):
    def __init__(
        ...
        self.k_p_signal = Signal(bits_sign=(width, True), reset=0)
        self.k_i_signal = Signal(bits_sign=(width, True), reset=0)
        self.setpoint_signal = Signal(bits_sign=(width, True), reset=0)
        self.prop = Signal(bits_sign=(2*width+1, True), reset=0)
        self.int = Signal(bits_sign=(2*width+1, True), reset=0)
        self.int_sum = Signal(bits_sign=(2*width+1, True), reset=0)
        self.sum = Signal(bits_sign=(2*width+2, True), reset=0)
